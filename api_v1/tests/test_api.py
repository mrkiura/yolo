...
class BucketlistActions(APITestCase):
    ...
    def test_bucketlist_access(self):
        self.client.post('/api/v1/bucketlists/', {'list_name': 'Another one'},
                         format='json')
    ...
    def test_unauthorised_bucketlist_access(self):
        response = self.client.post('/api/v1/auth/register/',
                                    ...
                                    format='json')
        gudaman_token = response.data['token']
        self.client.credentials(HTTP_AUTHORIZATION='JWT ' + gudaman_token)
...
class BucketlistItemActions(APITestCase):
    def setUp(self):
        auth_response = self.client.post('/api/v1/auth/register/',
                                         {'username': 'bond007',
                                          'password': 'nomasana',
